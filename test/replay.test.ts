...
describe("replay — spans & status", () => {
  it("computes one span and a finished status", () => {
    const log: Event[] = [
      ev({ type: "task.created", at: "2026-08-03T09:00:00+07:00", task: "auth", title: "Auth", imp: "high", tags: ["bug"] }),
  ...
  it("sums many spans across interruptions", () => {
    const log: Event[] = [
      ev({ type: "task.created", at: "2026-08-03T09:00:00+07:00", task: "a", title: "A", imp: "med", tags: [] }),
...
describe("replay — determinism", () => {
  ...
  it("drops duplicate ids (merge safety)", () => {
    const created = ev({ type: "task.created", at: "2026-08-03T09:00:00+07:00", task: "a", title: "A", imp: "med", tags: [] });
...
describe("replay — multitasking gross vs net", () => {
  it("counts gross per task but net once", () => {
    const log: Event[] = [
      ev({ type: "task.created", at: "2026-08-03T09:00:00+07:00", task: "a", title: "A", imp: "med", tags: [] }),
      ev({ type: "task.created", at: "2026-08-03T09:00:00+07:00", task: "b", title: "B", imp: "med", tags: [] }),
      ev({ type: "task.started", at: "2026-08-03T09:00:00+07:00", task: "a" }),
